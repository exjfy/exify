...
// ✅ Moved intro logic into a function
app.startIntro = () => {
      ...
      () => {
        ...
        writeLine(
          [
            ...
            `Welcome back, <i style='color: #0f0'>${ip}</i>! Nice to see someone from ${country}!`,
          ],
          30,
          500,
          () => {
...
const skipIntro = () => {
  ...
  $('#main').fadeOut(100, () => {
    ...
    setTimeout(() => $('.brand-header').animateCss(app.effects[Math.floor(Math.random() * app.effects.length)]), 200);
    ...
    setTimeout(() => {
      ...
      $('.marquee-container').css('visibility', 'visible').hide().fadeIn(100).animateCss('zoomIn');
      $('.container').fadeIn();
